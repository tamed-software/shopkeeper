...
function shopkeeper_entry_meta() {
	...
	// Post author
	if( $shopkeeper_theme_options['post_meta_author'] ) {
	...
	if( $shopkeeper_theme_options['post_meta_date'] ) {
	...
	if( $shopkeeper_theme_options['post_meta_categories'] ) {
...
function shopkeeper_entry_date( $echo = true ) {
	if ( has_post_format( array( 'chat', 'status' ) ) )
		$format_prefix = _x( '%1$s %2$s', '1: post format name. 2: date', 'shopkeeper' );
	else
		$format_prefix = '%2$s';

	$date = sprintf( __( '', 'shopkeeper' )
	);
...
function shopkeeper_post_header_entry_date( $echo = true ) {
	...
	$date = sprintf( '<a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s">%4$s</time></a>',
		...
		esc_attr( sprintf( __( 'Ir a %s', 'shopkeeper' ), the_title_attribute( 'echo=0' ) ) ),
